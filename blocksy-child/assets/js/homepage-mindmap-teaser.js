...
(function () {
	...
	if (!document.getElementById("hmt-teaser-styles")) {
		...
		style.textContent = [
			".hmt-root{background:#070709;border:1px solid rgba(255,176,32,.2);border-radius:14px;padding:28px 20px;margin:28px 0;}",
			".hmt-shell{max-width:980px;margin:0 auto;}",
			".hmt-kicker{margin:0 0 8px;color:#ffb020;text-transform:uppercase;letter-spacing:.08em;font-size:12px;font-weight:700;}",
			...
			".hmt-map{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;margin-top:22px;list-style:none;padding:0;}",
			".hmt-node{appearance:none;border:1px solid rgba(255,255,255,.14);border-top:3px solid var(--node-color);border-radius:12px;background:linear-gradient(180deg,rgba(255,255,255,.03),rgba(255,255,255,.015));color:#f8fafc;text-align:left;cursor:pointer;padding:14px 14px 12px;min-height:132px;position:relative;opacity:0;transform:translateY(12px);transition:border-color .25s ease,transform .25s ease;}",
			".hmt-node.is-mounted{animation:hmtFadeUp .42s ease forwards;}",
			".hmt-node:hover{border-color:rgba(255,176,32,.45);transform:translateY(-2px);box-shadow:0 16px 36px rgba(0,0,0,.35);}",
			".hmt-node:focus-visible{border-color:rgba(255,176,32,.6);transform:translateY(-2px);box-shadow:0 0 0 3px rgba(255,176,32,.22);outline:none;}",
			".hmt-node.is-active{border-color:rgba(255,176,32,.6);}",
			...
			".hmt-node:hover .hmt-tooltip,.hmt-node:focus-visible .hmt-tooltip,.hmt-node.is-active .hmt-tooltip{opacity:1;max-height:90px;}",
			...
			".hmt-node:focus-visible .hmt-stat,.hmt-node.is-active .hmt-stat{opacity:1;max-height:72px;transform:scale(1);}",
			".hmt-stat strong{color:#ffb020;font-size:1.35rem;line-height:1;margin-top:2px;}",
			".hmt-stat em{color:rgba(248,250,252,.82);font-size:.78rem;font-style:normal;margin-top:4px;}",
			".hmt-cta{display:inline-flex;margin-top:18px;color:#ffb020;text-decoration:none;font-weight:700;letter-spacing:.02em;}",
			".hmt-cta:hover{color:#ffc14d;}",
			...
			"@media (prefers-reduced-motion:reduce){.hmt-node{opacity:1;transform:none;transition:none;}.hmt-node.is-mounted{animation:none;}}",
		].join("");
	...
	function renderNode(phase, index) {
		var moduleItems = phase.modules
			...
			return (
				'<li class="hmt-map__item"><button type="button" class="hmt-node" data-node-id="' +
				phase.id +
				'" aria-label="' +
				phase.label +
				": " +
	...
	root.innerHTML =
		...
		'<p class="hmt-subtitle">Fundament aufbauen, Wachstum stabilisieren, Skalierung kontrolliert hochfahren.</p>' +
			'<ul class="hmt-map" aria-label="WGOS Teaser Mindmap">' +
